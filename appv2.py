...
if "current_challenge" not in st.session_state:
    ...
    st.session_state.is_custom_challenge = False
    st.session_state.custom_upload_name = None
    st.session_state.uploader_key_counter = 0

# --- Upload perso : si l'utilisateur dépose une photo, elle remplace le
# challenge courant et le modèle fera sa prédiction dessus (pas de vraie
# position connue dans ce cas). ---
uploaded_file = st.file_uploader(
    "📤 Ou teste avec ta propre photo (le modèle devinera où elle a été prise)",
    type=["jpg", "jpeg", "png"],
    key=f"custom_upload_{st.session_state.uploader_key_counter}"
)

if uploaded_file is not None and st.session_state.custom_upload_name != uploaded_file.name:
    ...
    st.session_state.is_custom_challenge = True
    st.session_state.custom_upload_name = uploaded_file.name
    st.session_state.result = None
    st.session_state.guessed_lat = None
    st.session_state.guessed_lon = None
    st.session_state.challenge_start_time = time.time()
...
if st.session_state.result is None:
    ...
    if elapsed < TIMER_DURATION:
        ...
        if st.session_state.is_custom_challenge:
            result = call_predict_api(
                st.session_state.current_challenge["image_bytes"],
                st.session_state.current_challenge["filename"]
            )
        else:
...
with col1:
    ...
    if st.session_state.is_custom_challenge:
        img_b64 = base64.b64encode(st.session_state.current_challenge["image_bytes"]).decode()
    else:
...
with col2:
    ...
    if st.session_state.result:
        if st.session_state.is_custom_challenge:
            machine_data = st.session_state.result
            folium.Marker(
...
if st.session_state.result:
    ...
    if st.session_state.is_custom_challenge:
        st.success("🔮 Voici la prédiction du modèle sur ta photo (marqueur rouge sur la carte) !")
    else:
